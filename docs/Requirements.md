...
1. возможность отключения от платформы вручную;
1. отключение от платформы по истечению определенного промежутка времени;
1. управление гусеничной платформой (управление движением):  
    - движение вперед;
    - движение назад;
...
1. получение информации о текущем состоянии сервоприводов:
...
в виде констант);  
1. получение корректной информации с датчиков расстояния (с "заводской" калибровкой датчиков 
в виде констант);  
1. обработка поврежденных и неподдерживаемых команд: при получении такой команды возвращать ошибку 
(см. [управляющие ответы](#service_answers));  
