...
## Взяимодействие с поворотной платформой
...
и получать информацию о текущем положении поворотной платформы.  
Предусловие: платформа находится в состоянии ожидания команды  
Основной поток событий:  
1. Вариант использования начинается, когда была принята и распознана команда работы с поворотной платформой  
2. Платформа проверяет корректность параметров в команде. При обнаружении некорректных параметров выполняется альтернативный поток E2.
3. Платформа проверяет вид команды. Если это команда о получении информации о текущем положении поворотной платформы, выполняется альтернативный поток событий A1.
...
1. Альтернативный поток событий A1 начинается.
...
4. Платформа возвращает [утвердительный ответ](Requirements.md#service_answers) используя внешний интерфейс, от которого было получено это сообщение.
5. Альтернативный поток событий A1 завершается
